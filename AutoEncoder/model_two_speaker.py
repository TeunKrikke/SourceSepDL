...
class LSTM_net(Network):
    ...
    def __init__(self, network, filter):
        """
            ...
            to do source separation.
        """
        super(LSTM, self).__init__(network)
        self.network = network
        self.filter = filter
...
class DNN_net(Network):
    ...
    def model(self):
        """
            ...
            model -- the network architecture.
        """
        speaker_1 = Input(shape=(None, self.features), name='speaker_1')
        speaker_2 = Input(shape=(None, self.features), name='speaker_2')
        mix = Input(shape=(None, self.features), name='mix')

        concat = Concatenate()([speaker_1, speaker_2])

        dnn_1 = TimeDistributed(Dense(self.features*2, activation='tanh'), name='dense_1')(concat)
        dnn_1 = TimeDistributed(Dense(900, activation='tanh'), name='dense_2')(dnn_1)
        dnn_1 = TimeDistributed(Dense(700, activation='tanh'), name='dense_3')(dnn_1)
        output_enc = TimeDistributed(Dense(self.features, activation='tanh'), name='ouput_enc')(dnn_1)
        input_dec = TimeDistributed(Dense(self.features, activation='linear'), name='input_dec')
        input_dec_linked = input_dec(output_enc)
        dnn_2 = TimeDistributed(Dense(700, activation='tanh'), name='dense_4')(input_dec_linked)
        dnn_2 = TimeDistributed(Dense(900, activation='tanh'), name='dense_5')(dnn_2)
        dnn_2 = TimeDistributed(Dense(self.features*2, activation='tanh'), name='dense_6')(dnn_2)
        pred_s1 = TimeDistributed(Dense(self.features, activation='tanh'), name='pred_s1')(dnn_2)
        pred_s2 = TimeDistributed(Dense(self.features, activation='tanh'), name='pred_s2')(dnn_2)

        model = Model(input=[speaker_1, speaker_2], output=[pred_s1, pred_s2, output_enc])
        encoder = Model(input=[speaker_1, speaker_2], output=[output_enc])
        input_dec_linked = input_dec(mix)
        decoder = Model(input=[mix], output=[pred_s1, pred_s2])
